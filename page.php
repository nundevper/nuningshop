...
<html xmlns="http://www.w3.org/1999/xhtml"><!-- InstanceBegin template="/Templates/font.dwt.php" codeOutsideHTMLIsLocked="false" -->
	<head>
        ...
        $id=intval($_GET['id']);
        $sqlDetail="SELECT * FROM page WHERE id=$id";
        $resultDetail=mysql_query($sqlDetail)or die(mysql_error());
        $rowDetail=mysql_fetch_array($resultDetail);
        if(mysql_num_rows($resultDetail)==0){echo"<script>window.location='$rowSetting[website_name]';</script>";exit();}
        ?>
        <title><?=$rowDetail['name']?> | <?=$rowSetting['title']?></title>
        <meta name="description" content="<?=mb_substr(strip_tags($rowDetail['detail']),0,200,'UTF-8')?>"/>
		<!-- InstanceEndEditable -->
...
<div class="fb-page" data-href="<?=$rowSetting['facebook_fanpage']?>" data-small-header="false" data-adapt-container-width="true" data-hide-cover="false" data-show-facepile="true"></div>
        ...
        <div class="col-md-9 col-sm-8 col-xs-12" id="zpage">
        	<div class="col-md-12 col-sm-12 col-xs-12 box">
                <h3 class="box-header" style="text-align:left;"><?=$rowDetail['name']?></h3>
                <div class="box-content">
					<?=$rowDetail['detail']?>
                </div>
            </div>
...
?>
<a href="admin/pageEdit.php?id=<?=base64_encode($id)?>" id="edit" target="_blank" title="แก้ไข"><img src="admin/image/edit.png"/></a>
<?	
}
?>
<script type="text/javascript">
