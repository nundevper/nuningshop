...
<?
if(empty($_GET['id'])){echo"<script>window.location='เว็บบอร์ด-1.html';</script>";}
//หัวข้อ
$id=intval($_GET['id']);
...
if(mysql_num_rows($resultPost)==0){echo"<script>window.location='เว็บบอร์ด-1.html';</script>";}
//ตอบ
...
?>
<title><?=$rowPost['subject']?> | <?=$rowSetting['shop_name']?></title>
<meta name="description" content="<?=mb_substr(strip_tags($rowPost['detail']),0,200,'UTF-8')?>"/>
<!-- InstanceEndEditable -->
...
<table class="box" border="0" cellspacing="0" cellpadding="10">
  <tr>
    <th style="text-align:left;">ตอบหัวข้อนี้</th>
  </tr>
  <tr>
    <td>
<form method="post" action="process/reply.php">
<table width="100%" border="0" cellspacing="0" cellpadding="5">
  ...
  <tr>
    <td>ชื่อ</td>
    <td>
      <input type="text" name="username" pattern="[a-zA-Z0-9ก-๙_-]{3,50}" required value="<?=$username?>"/>
      <input type="text" name="name" style="display:none;"/>
    </td>
  </tr>
  <tr>
    <td><input type="hidden" name="post_id" value="<?=$id?>"/></td>
...
?>
<a href="admin/webboardEdit.php?id=<?=base64_encode($id)?>" id="edit" target="_blank" title="แก้ไข"><img src="admin/image/edit.png"/></a>
<?	
}
?>
<script type="text/javascript">
