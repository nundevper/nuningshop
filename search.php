...
<title>ค้นหาสินค้า | <?=$rowSetting['title']?></title>
...
<div class="fb-page" data-href="<?=$rowSetting['facebook_fanpage']?>" data-small-header="false" data-adapt-container-width="true" data-hide-cover="false" data-show-facepile="true"></div>
        ...
        <div class="col-md-9 col-sm-8" id="product-list">	
			...
			$sql="SELECT SQL_CALC_FOUND_ROWS * FROM product WHERE name LIKE '%".mysql_escape_string($search)."%' OR keyword LIKE '%".$search."%' ORDER BY pin DESC, id DESC LIMIT $start_row, $rows_per_page";
            ...
            <div class="col-md-12 col-sm-12 col-xs-12" id="pagination">
                ...
                if($current_page>1){$pg=$current_page-1;
                  echo"<a href='ค้นหาสินค้า-$s-$pg.html' class='button' title='หน้า $pg'>ย้อนกลับ</a>&nbsp;";
                }
                if($page_start>1){$pg=$page_start-1;
                  echo"<a href='ค้นหาสินค้า-$s-$pg.html' class='number' title='หน้า $pg'>...</a>";
                }
                //แสดงหมายเลขเพจ
                for($i=$page_start;$i<=$page_end;$i++){
                  if($i==$current_page){echo"<span class='number-selected'>$i</span>";}
                  else{echo"<a href='ค้นหาสินค้า-$s-$i.html' class='number' title='หน้า $i'>$i</a>";}
                }
                //ถัดไป
                if($page_end<$total_pages){
                  $pg=$page_end+1;
                ...
                if($current_page<$total_pages){
                  $pg=$current_page+1;
                  echo"&nbsp;<a href='ค้นหาสินค้า-$s-$pg.html' class='button' title='หน้า $pg'>ถัดไป</a>";
                }
                ?>
...
<table class="box" border="0" cellspacing="0" cellpadding="10">
  <tr>
    <th style="text-align:left;"><?=$total_rows?> ผลการค้นหา</th>
  </tr>
  <tr>
    <td>
<form name="search" method="post" action="ค้นหาสินค้า.html" style="text-align:right;">
<input type="text" name="search" pattern="[a-zA-Z0-9ก-๙]+" required autofocus title="ห้ามมีอักขระพิเศษ" value="<?=$search?>"/>
<input type="submit" value="ค้นหา" />
</form>
    </td>
  </tr>
